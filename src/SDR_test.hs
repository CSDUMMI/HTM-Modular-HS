...
sdr_test = do 
  ...
  let sdr_2 = SDR {
        sdr_active_indicies=[9,11,10],
        len=16
       }
  let union_fail = s_union sdr_1 sdr_to_fail
  expect $ union_fail `toBe` sdr_1
  expect $ (s_union sdr_1 sdr_2) `pred_compare_sdrs` SDR {
    sdr_active_indicies=[9,11,8,10],
    len=16
    }
